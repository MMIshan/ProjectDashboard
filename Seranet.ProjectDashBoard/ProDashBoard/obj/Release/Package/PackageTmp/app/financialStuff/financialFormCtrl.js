(function () {
    ...
    app.controller("financialDataController", ['$scope', 'toaster', '$mdDialog', '$http', '$stateParams', FinancialDataController]);

    function FinancialDataController($scope,toaster,$mdDialog, $http, $stateParams) {
        $scope.yearArray = [];
        ...
        function loadTimeReportData(year,month,accountId) {
            ...
            $http.get('api/FinancialData/getDataFromTimeReports/' + year + '/' + month + '/' + accountId).success(function (data) {
                $scope.timeReportData = data;
                if ($scope.timeReportData.length != 0) {
                    console.log("Time Report Data Retrieved " + data.length);
                    angular.forEach(data, function (value, key) {
                        console.log(value.EmpId + " " + value.EmpName + " " + value.AllocatedHours + " " + value.BillableHours + " " + value.TotalHours);
        ...
        function loadDate() {
            ...
            var year = 2014;
            //$scope.yearCombo1 = parseInt(2015);
            $scope.monthCombo =JSON.stringify(currentMonth-1);
            $scope.yearCombo1 =2011;
        ...
        $scope.accountChange = function () {
            ...
            //$scope.yearCombo1 = 2015;
            //alert(receivedObject.AccountName);
        ...
        $scope.SaveData = function (ev) {
            ...
            angular.forEach($scope.timeReportData, function (value, key) {
                ...
                console.log("Data " + " " + value.EmpName + " " + month + " " + accountId + " " + $scope.billableCheck[value.EmpId] + " " + $scope.AllocatedHours[value.EmpId] + " " + $scope.BillableHours[value.EmpId] + " " + $scope.TotalHours[value.EmpId] + " " + $scope.billableCheck[value.EmpId]);
