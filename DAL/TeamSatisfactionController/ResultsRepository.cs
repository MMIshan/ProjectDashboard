        ...
        public List<List<object[]>> getSelectedResults(int projectid,int year,int quarter)
        {
            ...
            if (displayingQuestion.Count != 0)
            {
                ...
                foreach (Questions q1 in displayingQuestion)
                {
                    ...
                    String query= @"select m.id,m.MemberName,q.QuestionName,r.Answer,r.Year,r.Quarter from results r, Questions q,TeamMembers m where r.QuestionID = q.Id and r.MemberId = m.Id and r.AccountID = '" + projectid + "' and q.CalcExist = 1 and r.QuestionID = '" + q1.Id + "' and r.Year = (select top(1) PERCENT r.Year from results r, Questions q where r.QuestionID = q.Id and r.AccountID = '" + projectid + "' and q.CalcExist = 1 order by year desc, quarter desc) and r.Quarter = (select top(1) PERCENT r.quarter from results r, Questions q where r.QuestionID = q.Id and r.AccountID = '" + projectid + "' and q.CalcExist = 1 order by year desc, quarter desc) order by m.id asc";
